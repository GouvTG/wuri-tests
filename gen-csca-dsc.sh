...
${OPENSSL} req -x509 \
	-subj '/CN=National CSCA of TOGO/C=TG/' \
	-key csca.key \
	-out csca.pem -nodes \
	-days 3650
...
${OPENSSL} req -new -subj "/CN=DSC number $i of TOGO/C=TG/" -key dsc-$i.key -nodes |
${OPENSSL} x509 -req -CA csca.pem -CAkey csca.key -set_serial 0x$R -days 1780 -out dsc-$i.pem
done
